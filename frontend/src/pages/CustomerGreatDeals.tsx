...
const CustomerGreatDeals = () => {
  ...
  const loadGreatDeals = async () => {
    try {
      ...
      // Filter to only show dishes with active discounts (discount > 0)
      const dealsWithDiscounts = data.filter((dish: any) => dish.discount && dish.discount > 0);

      setDishes(dealsWithDiscounts);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {dishes.map((dish: any) => (
            <div
              ...
              onClick={() => {
                // Navigate to restaurant's menu page with the restaurant selected
                if (dish.restaurantId && dish.restaurantId._id) {
                  navigate('/customer/home', { state: { selectedRestaurant: dish.restaurantId } });
                }
              }}
            >
              {/* Discount Badge */}
